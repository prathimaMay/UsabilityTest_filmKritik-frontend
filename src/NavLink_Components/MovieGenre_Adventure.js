...
import MovieGenreAdventureDetails from "./MovieGenreAdventure_Details";
import {setCookieRate, getCookieRate } from '../Cookies';

const MovieGenreAdventure = () => {   
...
return(    
    ...
    {movies.length > 0 && movies.slice(0,10).map((movie) => 
        <MovieGenreAdventureDetails key={movie.id} {...movie} movie_id={movie_id} new_rating={new_rating} />)}
 </div>
);
}

export default MovieGenreAdventure;